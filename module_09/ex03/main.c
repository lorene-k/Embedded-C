...
int main() {
    ...
    write_data(OUTPUT_0, 0b01111111);       // Set DP4 as output (off)
    write_data(CONF_1, 0b10000000);         // Set segments a, b, g, e, d as outputs
    write_data(OUTPUT_1, 0b00111111);       // Start with segments off
    uint8_t i = 0;
    while (1) {
        write_data(OUTPUT_1, segments[i]);
        _delay_ms(1000);
        i++;
        if (i > 9)
            i = 0;
